...
class CRF(nn.Module):
    ...
    def __init__(self, in_features, num_tags, start_idx, stop_idx):
    ...
    def loss(self, features, target):
        ...
        forward_score = self.__forward_algorithm(features, masks_)
        gold_score = self.__score_sentence(features, target[:, :L].long(), masks_)
    ...
    def viterbi_decode(self, features, masks):
        ...
        for t in range(L):
            ...
            # [B, 1, C] + [C, C]
            acc_score_t = max_score.unsqueeze(1) + self.transitions.to(features.device)  # [B, C, C]
            acc_score_t, bps[:, t, :] = acc_score_t.max(dim=-1)
            acc_score_t += emit_score_t
            max_score = acc_score_t * mask_t + max_score * (1 - mask_t)  # max_score or acc_score_t

        # Transition to STOP_TAG
        max_score += self.transitions.to(features.device)[self.stop_idx]
