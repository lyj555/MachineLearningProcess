...
def xgb_explain(data, model, top_feature_num, strategy, prob_threshold=0.5, feature_group=None,
                if_sort_group_by_feature_importance=False, feature_meaning_dic={}, verbal_express=None,
                objective="binary"):
    """
    use shap explain xgboost model, for now only support binary classification
    :param data: pandas.DataFrame, data need to predict
    :param model: xgb.Booster or sklearn's xgb, xgboost's model
    ...
    :param objective: str[binary], only support binary, default binary
    :return: list[str], the length = len(data)
    """
    assert isinstance(data, pd.DataFrame), "input data must be pandas.DataFrame"
    assert isinstance(model, xgb.core.Booster) or isinstance(model, xgb.XGBModel), \
        "input model must be xgboost's model"
    ...
    if objective == "binary":
        ...
        if if_sort_group_by_feature_importance:
            feature_importances = np.array([model.get_fscore()[i] for i in cols])
        ...
        shap_values = explainer.shap_values(data)  # len(data)*len(feat)
...
if __name__ == "__main__":
    ...
    #     reasons = xgb_explain(X_test, model, top_feature_num=3, strategy=i, feature_group=None)
    #     print(reasons)
    # test feature group
    ...
    # reasons = xgb_explain(X_test, model, top_feature_num=3, strategy="max_shap", feature_group=feature_group)
    # reasons = xgb_explain(X_test, model, top_feature_num=3, strategy="max_shap",
    ...
    def test_verbal_express(feature_value_pair, feature_meaning_dic):
        ret = []
        for feature, feature_value in feature_value_pair:
            feature_meaning = feature_meaning_dic.get(feature, feature)  # if no this key meaning, use key instead
            ret.append(f"{feature_meaning} value is {feature_value}")
