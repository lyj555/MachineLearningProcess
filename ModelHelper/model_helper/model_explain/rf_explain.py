...
def rf_explain(data, model, top_feature_num, strategy, prob_threshold=0.5, feature_group=None,
               if_sort_group_by_feature_importance=False, feature_meaning_dic={}, verbal_express=None,
               objective="binary"):
    """
    use shap explain random forest model, for now only support binary classification
    :param data: pandas.DataFrame, data need to predict
    :param model: xgb.Booster or sklearn's RandomForestClassifier
    ...
    assert isinstance(model, RandomForestClassifier), "input model must be RandomClassifier"
...
if __name__ == "__main__":
    ...
    for i in ["max_shap", "min_shap", "abs_shap", "threshold"]:
        reasons = rf_explain(X_test, model, top_feature_num=3, strategy=i, feature_group=None)
        print(reasons)
    # test feature group
    ...
    # reasons = rf_explain(X_test, model, top_feature_num=3, strategy="max_shap", feature_group=feature_group)
    # reasons = rf_explain(X_test, model, top_feature_num=3, strategy="max_shap",
