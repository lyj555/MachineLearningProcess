...
from .ensemble_base import StackingBase
from ..utils.common_utils import check_X_y
from ..utils.constants import JobType, ModelStage


class StackingClassifier(StackingBase, ClassifierMixin):
    ...
    Parameters
    ----------
    :param k_fold: int or splitter object(KFold or StratifiedKFold), used to split training data, default 5.
    ...
    def fit(self, X, y, stratify=False, stratify_col=None):
        """
        method to train StackingClassifier
        ...
        :return: self
        """
        assert (isinstance(self.k_fold, int) and self.k_fold >= 2) or \
               isinstance(self.k_fold, KFold) or isinstance(self.k_fold, StratifiedKFold), \
               "param k_fold must be int and geq 2 or KFold or StratifiedKFold object!"
...
class StackingRegressor(StackingBase, RegressorMixin):
    ...
    def predict(self, X):
        X = check_X_y(X=X, y=None, stage=ModelStage.PREDICT, job=JobType.REGRESSION, feature_info=self._feature_info)
