...
def _get_shap_matrix(img, model, ith_predict, background_sample):
...
def explain_image_plot(images, model, background_sample, filter_blank=False, blank_index_list=None,
                       row_scale=3, col_scale=3, index_map=None, fontsize=10, back_alpha=0.15, hspace=0.2, aspect=0.2,
                       show=True):
    """
    explain model images
    :param images: numpy.array, (batch_size, ...)
    :param model: keras model,
    :param background_sample: numpy.array, (batch_size, ...)
    ...
    if len(output_dimension) == 2:  # self-define explain plot, default shap can not process this.
        ...
        for row_index in range(row_num):
            ...
            x_curr = images[row_index].copy()  # current image
            ...
            axes[row_index, 0].imshow(x_curr, cmap=plt.get_cmap('gray'))  # plot the raw image with gray style
        ...
        # model output dimension < 2, can use default shap function.
        e = shap.DeepExplainer(model, background_sample)
        shap_values = e.shap_values(images, ranked_outputs=1)
