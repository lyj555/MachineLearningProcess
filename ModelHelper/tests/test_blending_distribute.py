...
from model_helper.model_ensemble import BlendingClassifier, BlendingRegressor
...
def process_file(spark, local_file_path):
    ...
    print("TEST BlendingClassifier...")
    print("test1 test normal process...")
    clf = BlendingClassifier(base_learner_list=[RandomForestClassifier(), GradientBoostingClassifier(),
    ...
    print("TEST BlendingClassifier SUCC.")

    print("TEST BlendingRegressor...")
    X, y = make_regression(n_samples=5000, n_features=20, random_state=224)
    train_x, test_x, train_y, test_y = train_test_split(X, y, test_size=0.3)

    # test1 test normal process
    print("test1 test normal process...")
    clf = BlendingRegressor(base_learner_list=[RandomForestRegressor(), GradientBoostingRegressor(),
    ...
    clf = BlendingRegressor(base_learner_list=[RandomForestRegressor(), GradientBoostingRegressor(),
                            ...
                            meta_learner=LinearRegression(),
                            metric_func=lambda y_true, y_score: r2_score(y_true=y_true, y_pred=y_score),
                            select_base_learner=selector, feature_fraction=0.8, distribute=True, spark=spark)
    clf.fit(X=train_x, y=train_y)

    pred = clf.predict(X=test_x)
    r2_val = r2_score(y_true=test_y, y_pred=pred)
    print(f"test3 done, r2 val is {r2_val}")
    print("TEST BlendingRegressor SUCC.")
